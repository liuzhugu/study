...
public abstract class ClassValue_<T> {
    /**
     * Sole constructor.  (For invocation by subclass constructors, typically
     * implicit.)
     */
    protected ClassValue_() {
    }

    /**
     * Computes the given class's derived value for this {@code classValue_}.
     * <p>
     ...
     * @return the newly computed value associated with this {@code classValue_}, for the given class or interface
     * @see #get
     * @see #remove
     */
    protected abstract T computeValue(Class_<?> type);
     ...
     * @return the current value associated with this {@code classValue_}, for the given class or interface
     * @throws NullPointerException if the argument is null
     * @see #remove
     * @see #computeValue
     */
    public T get(Class_<?> type) {
        // non-racing this.hashCodeForCache : final int
        ClassValue_.Entry<?>[] cache = getCacheCarefully(type);
        //ClassValue_.Entry<T> e = probeHomeLocation(cache = getCacheCarefully(type), this);
        ClassValue_.Entry<T> e = null;
     ...
     * @throws NullPointerException if the argument is null
     */
    public void remove(Class_<?> type) {
        ClassValue_.ClassValueMap map = getMap(type);
        map.removeEntry(this);
    }

    // Possible functionality for JSR 292 MR 1
    /*public*/ void put(Class_<?> type, T value) {
        ClassValue_.ClassValueMap map = getMap(type);
    ...
    private static ClassValue_.Entry<?>[] getCacheCarefully(Class_<?> type) {
        // racing type.classValueMap{.cacheArray} : null => new Entry[X] <=> new Entry[Y]
        ClassValue_.ClassValueMap map = type.classValueMap;
        if (map == null)  return EMPTY_CACHE;
        ClassValue_.Entry<?>[] cache = map.getCache();
        return cache;
    ...
    private static final ClassValue_.Entry<?>[] EMPTY_CACHE = { null };

    /**
     * Slow tail of classValue_.get to retry at nearby locations in the cache,
     ...
     */
    private T getFromBackup(ClassValue_.Entry<?>[] cache, Class_<?> type) {
        //ClassValue_.Entry<T> e = probeBackupLocations(cache, this);
        ClassValue_.Entry<T> e = null;
        if (e != null)
            return e.value();
    ...
    ClassValue_.Entry<T> castEntry(ClassValue_.Entry<?> e) { return (ClassValue_.Entry<T>) e; }

    /** Called when the fast path of get fails, and cache reprobe also fails.
     */
    private T getFromHashMap(Class_<?> type) {
        // The fail-safe recovery is to fall back to the underlying classValueMap.
        ClassValue_.ClassValueMap map = getMap(type);
        for (;;) {
            ClassValue_.Entry<T> e = map.startEntry(this);
            if (!e.isPromise())
                return e.value();
            try {
    ...
    /** Check that e is non-null, matches this classValue_, and is live. */
    boolean match(ClassValue_.Entry<?> e) {
    ...
    /**
     * This classValue_'s identity, expressed as an opaque object.
     * The main object {@code classValue_.this} is incorrect since
     * subclasses may override {@code classValue_.equals}, which
     * could confuse keys in the ClassValueMap.
     */
    final ClassValue_.Identity identity = new ClassValue_.Identity();
     ...
     * A version change invalidates all cache entries for the affected classValue_,
     ...
     * <p>
     * All user-visible state changes on the classValue_ take place under
     * a lock inside the synchronized methods of ClassValueMap.
     * Readers (of classValue_.get) are notified of such state changes
     ...
     * before this.version.
     */
    private volatile ClassValue_.Version<T> version = new ClassValue_.Version<>(this);
    ClassValue_.Version<T> version() { return version; }
    void bumpVersion() { version = new ClassValue_.Version<>(this); }
    static class Version<T> {
        private final ClassValue_<T> classValue_;
        private final ClassValue_.Entry<T> promise = new ClassValue_.Entry<>(this);
        Version(ClassValue_<T> classValue_) { this.classValue_ = classValue_; }
        ClassValue_<T> classValue_() { return classValue_; }
        ClassValue_.Entry<T> promise() { return promise; }
        boolean isLive() { return classValue_.version() == this; }
    }

    /** One binding of a value to a class via a classValue_.
     *  States are:<ul>
     *  <li> promise if value == Entry.this
     *  <li> else dead if version == null
     *  <li> else stale if version != classValue_.version
     *  <li> else live </ul>
     ...
     *  into the dead state.
     */
    static class Entry<T> extends WeakReference<ClassValue_.Version<T>> {
        final Object value;  // usually of type T, but sometimes (Entry)this
        Entry(ClassValue_.Version<T> version, T value) {
            super(version);
        ...
        Entry(ClassValue_.Version<T> version) {
            super(version);
        ...
        ClassValue_.Version<T> version() { return get(); }
        ClassValue_<T> classValueOrNull() {
            ClassValue_.Version<T> v = version();
            return (v == null) ? null : v.classValue_();
        }
        boolean isLive() {
            ClassValue_.Version<T> v = version();
        ...
        ClassValue_.Entry<T> refreshVersion(ClassValue_.Version<T> v2) {
            assertNotPromise();
            @SuppressWarnings("unchecked")  // if !isPromise, type is T
                    ClassValue_.Entry<T> e2 = new ClassValue_.Entry<>(v2, (T) value);
            clear();
            // value = null -- caller must drop
            return e2;
        }
        static final ClassValue_.Entry<?> DEAD_ENTRY = new ClassValue_.Entry<>(null, null);
    }

    /** Return the backing map associated with this type. */
    private static ClassValue_.ClassValueMap getMap(Class_<?> type) {
    ...
    private static ClassValue_.ClassValueMap initializeMap(Class_<?> type) {
        ClassValue_.ClassValueMap map;
        synchronized (CRITICAL_SECTION) {  // private object to avoid deadlocks
            // happens about once per type
            if ((map = type.classValueMap) == null)
                type.classValueMap = map = new ClassValue_.ClassValueMap(type);
        }
        return map;
    }

    static <T> ClassValue_.Entry<T> makeEntry(ClassValue_.Version<T> explicitVersion, T value) {
        // Note that explicitVersion might be different from this.version.
        return new ClassValue_.Entry<>(explicitVersion, value);
     ...
     *  Gives a fully serialized "true state" for each pair (classValue_ cv, Class type).
     *  Also manages an unserialized fast-path cache.
     */
    static class ClassValueMap extends WeakHashMap<ClassValue_.Identity, ClassValue_.Entry<?>> {
        private final Class_<?> type;
        private ClassValue_.Entry<?>[] cacheArray;
        private int cacheLoad, cacheLoadLimit;

        /** Number of entries initially allocated to each type when first used with any classValue_.
         ...
         */
        ClassValueMap(Class_<?> type) {
            this.type = type;
            sizeCache(INITIAL_ENTRIES);
        }

        ClassValue_.Entry<?>[] getCache() { return cacheArray; }

        /** Initiate a query.  Store a promise (placeholder) if there is no value yet. */
        synchronized
        <T> ClassValue_.Entry<T> startEntry(ClassValue_<T> classValue_) {
            @SuppressWarnings("unchecked")  // one map has entries for all value types <T>
                    ClassValue_.Entry<T> e = (ClassValue_.Entry<T>) get(classValue_.identity);
            ClassValue_.Version<T> v = classValue_.version();
            if (e == null) {
                e = v.promise();
                // The presence of a promise means that a value is pending for v.
                // Eventually, finishEntry will overwrite the promise.
                put(classValue_.identity, e);
        ...
        synchronized
        <T> ClassValue_.Entry<T> finishEntry(ClassValue_<T> classValue_, ClassValue_.Entry<T> e) {
            @SuppressWarnings("unchecked")  // one map has entries for all value types <T>
                    ClassValue_.Entry<T> e0 = (ClassValue_.Entry<T>) get(classValue_.identity);
            if (e == e0) {
                // We can get here during exception processing, unwinding from computeValue.
                assert(e.isPromise());
                remove(classValue_.identity);
                return null;
        ...
        /** Remove an entry. */
        synchronized
        void removeEntry(ClassValue_<?> classValue_) {
            ClassValue_.Entry<?> e = remove(classValue_.identity);
            if (e == null) {
                ...
                classValue_.bumpVersion();
                // Make all cache elements for this guy go stale.
                removeStaleEntries(classValue_);
            }
        }

        /** Change the value for an entry. */
        synchronized
        <T> void changeEntry(ClassValue_<T> classValue_, T value) {
            @SuppressWarnings("unchecked")  // one map has entries for all value types <T>
                    ClassValue_.Entry<T> e0 = (ClassValue_.Entry<T>) get(classValue_.identity);
            ClassValue_.Version<T> version = classValue_.version();
            if (e0 != null) {
            ...
            ClassValue_.Entry<T> e = makeEntry(version, value);
            put(classValue_.identity, e);
            // Add to the cache, to enable the fast path, next time.
            checkCacheLoad();
            addToCache(classValue_, e);
        ...
        static ClassValue_.Entry<?> loadFromCache(ClassValue_.Entry<?>[] cache, int i) {
        ...
        /** Look in the cache, at the home location for the given classValue_. */
        static <T> ClassValue_.Entry<T> probeHomeLocation(ClassValue_.Entry<?>[] cache, ClassValue_<T> classValue_) {
            return classValue_.castEntry(loadFromCache(cache, classValue_.hashCodeForCache));
        }

        /** Given that first probe was a collision, retry at nearby locations. */
        static <T> ClassValue_.Entry<T> probeBackupLocations(ClassValue_.Entry<?>[] cache, ClassValue_<T> classValue_) {
            if (PROBE_LIMIT <= 0)  return null;
            // Probe the cache carefully, in a range of slots.
            int mask = (cache.length-1);
            int home = (classValue_.hashCodeForCache & mask);
            ClassValue_.Entry<?> e2 = cache[home];  // victim, if we find the real guy
            if (e2 == null) {
                return null;   // if nobody is at home, no need to search nearby
            }
            // assume !classValue_.match(e2), but do not assert, because of races
            int pos2 = -1;
            for (int i = home + 1; i < home + PROBE_LIMIT; i++) {
                ClassValue_.Entry<?> e = cache[i & mask];
                if (e == null) {
                    break;   // only search within non-null runs
                }
                if (classValue_.match(e)) {
                    // relocate colliding entry e2 (from cache[home]) to first empty slot
                    cache[home] = e;
                    if (pos2 >= 0) {
                        cache[i & mask] = ClassValue_.Entry.DEAD_ENTRY;
                    } else {
                        pos2 = i;
                    }
                    cache[pos2 & mask] = ((entryDislocation(cache, pos2, e2) < PROBE_LIMIT)
                            ? e2                  // put e2 here if it fits
                            : ClassValue_.Entry.DEAD_ENTRY);
                    return classValue_.castEntry(e);
                }
        ...
        private static int entryDislocation(ClassValue_.Entry<?>[] cache, int pos, ClassValue_.Entry<?> e) {
            ClassValue_<?> cv = e.classValueOrNull();
        ...
        private void sizeCache(int length) {
            ...
            cacheArray = new ClassValue_.Entry<?>[length];
        }
        ...
        private void reduceCacheLoad() {
            ...
            if (cacheLoad < cacheLoadLimit)
                return;  // win
            ClassValue_.Entry<?>[] oldCache = getCache();
            if (oldCache.length > HASH_MASK)
                return;  // lose
            sizeCache(oldCache.length * 2);
            for (ClassValue_.Entry<?> e : oldCache) {
                if (e != null && e.isLive()) {
                    addToCache(e);
                }
            }
         ...
         */
        private void removeStaleEntries(ClassValue_.Entry<?>[] cache, int begin, int count) {
            ...
            for (int i = begin; i < begin + count; i++) {
                ...
                ClassValue_.Entry<?> replacement = null;
                if (PROBE_LIMIT > 1) {
         ...
         */
        private ClassValue_.Entry<?> findReplacement(ClassValue_.Entry<?>[] cache, int home1) {
            ClassValue_.Entry<?> replacement = null;
            int haveReplacement = -1, replacementPos = 0;
            int mask = (cache.length-1);
            for (int i2 = home1 + 1; i2 < home1 + PROBE_LIMIT; i2++) {
                ClassValue_.Entry<?> e2 = cache[i2 & mask];
            ...
            if (haveReplacement >= 0) {
                if (cache[(replacementPos+1) & mask] != null) {
                    ...
                    cache[replacementPos & mask] = (ClassValue_.Entry<?>) ClassValue_.Entry.DEAD_ENTRY;
                } else {
        ...
        /** Remove stale entries in the range near classValue_. */
        private void removeStaleEntries(ClassValue_<?> classValue_) {
            removeStaleEntries(getCache(), classValue_.hashCodeForCache, PROBE_LIMIT);
        }

        /** Remove all stale entries, everywhere. */
        private void removeStaleEntries() {
            ClassValue_.Entry<?>[] cache = getCache();
            removeStaleEntries(cache, 0, cache.length + PROBE_LIMIT - 1);
        }

        /** Add the given entry to the cache, in its home location, unless it is out of date. */
        private <T> void addToCache(ClassValue_.Entry<T> e) {
            ClassValue_<T> classValue_ = e.classValueOrNull();
            if (classValue_ != null)
                addToCache(classValue_, e);
        }

        /** Add the given entry to the cache, in its home location. */
        private <T> void addToCache(ClassValue_<T> classValue_, ClassValue_.Entry<T> e) {
            if (PROBE_LIMIT <= 0)  return;  // do not fill cache
            // Add e to the cache.
            ClassValue_.Entry<?>[] cache = getCache();
            int mask = (cache.length-1);
            int home = classValue_.hashCodeForCache & mask;
            ClassValue_.Entry<?> e2 = placeInCache(cache, home, e, false);
         ...
         */
        private ClassValue_.Entry<?> placeInCache(ClassValue_.Entry<?>[] cache, int pos, ClassValue_.Entry<?> e, boolean gently) {
            ClassValue_.Entry<?> e2 = overwrittenEntry(cache[pos]);
            if (gently && e2 != null) {
         ...
         *  in its place.
         */
        private <T> ClassValue_.Entry<T> overwrittenEntry(ClassValue_.Entry<T> e2) {
