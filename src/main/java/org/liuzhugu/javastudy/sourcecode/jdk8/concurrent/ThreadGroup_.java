...
public class ThreadGroup_ implements Thread_.UncaughtExceptionHandler {
     ...
     * @see     java.lang.ThreadGroup_#checkAccess()
     * @since   JDK1.0
     */
    ...
    private int enumerate(ThreadGroup_ list[], int n, boolean recurse) {
        ...
        synchronized (this) {
            ...
            if (ng > 0) {
                System_.arraycopy(groups, 0, list, n, ng);
                n += ng;
            }
            if (recurse) {
                ngroupsSnapshot = ngroups;
    ...
    private void remove(ThreadGroup_ g) {
        synchronized (this) {
            ...
            for (int i = 0 ; i < ngroups ; i++) {
                if (groups[i] == g) {
                    ngroups -= 1;
                    System_.arraycopy(groups, i + 1, groups, i, ngroups - i);
    ...
    private void remove(Thread_ t) {
        synchronized (this) {
            ...
            for (int i = 0 ; i < nthreads ; i++) {
                if (threads[i] == t) {
                    System_.arraycopy(threads, i + 1, threads, i, --nthreads - i);
     ...
     *     printed to the {@linkplain System_#err standard error stream}.
     * </ul>
     * <p>
    ...
    public void uncaughtException(Thread_ t, Throwable e) {
        if (parent != null) {
            ...
            if (ueh != null) {
                ...
                System_.err.print("Exception in thread \""
                        + t.getName() + "\" ");
                e.printStackTrace(System_.err);
            }
        }
    }
